...
class ScoreNet(nn.Module):
    ...
    def resize_image(self, images):
        # Compute new dimensions that are multiples of self.large_patch_size
        new_height = (images.shape[2] // self.large_patch_size) * self.large_patch_size
        new_width = (images.shape[3] // self.large_patch_size) * self.large_patch_size
        
        # Resize the images using F.interpolate
        resized_images = F.interpolate(images, size=(new_height, new_width), mode='bilinear', align_corners=False)
        return resized_images

    # Example usage within the model's forward method or preprocessing pipeline
    def forward(self, x, use_hard_topk):
        # Resize the image using the function
